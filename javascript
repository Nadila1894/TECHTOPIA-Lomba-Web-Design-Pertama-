...
const navItems = document.querySelectorAll(".navbar-group > .navbar-item");

navItems.forEach((item) => {
    // Hanya jalankan logika hover di desktop
    if (window.innerWidth > 1024) {
        item.addEventListener("mouseenter", () => {
            const simpleDown = item.querySelector(".navbar-down");
            const megaMenu = item.querySelector(".mega-menu");

            if (megaMenu) {
                megaMenu.classList.add("show");
            } else if (simpleDown) {
                simpleDown.classList.add("navbar-expand");
            }
        });
        ...
        item.addEventListener("mouseleave", () => {
            ...
            if (megaMenu) {
                ...
                simpleDown.classList.remove("navbar-expand");
            }
        });
    }
});
